...
<table class="table table-borderless">
  ...
  <table class="table table-bordered table-striped table-sm pt-3">
    ...
    <tbody>
        <tr>
            <td>Uang Lembur</td>
            <td class="text-right">
            Rp. {{ $data->total_uang_lembur > 0 ? number_format($data->total_uang_lembur, 0, ',', '.') : '-' }}
            </td>
        </tr>
        <tr>
            <td>Do'a</td>
        ...
        <tr>
            <td>Premi Lembur</td>
            <td class="text-right">
            Rp. {{ $data->premi_lembur > 0 ? number_format($data->premi_lembur, 0, ',', '.') : '-' }}
            </td>
        </tr>
        ...
        @if(strtolower($data->employee->departemen) == 'packing')
            ...
            <td>Kopi</td>
            <td class="text-right">
                Rp. {{ $data->total_uang_kopi > 0 ? number_format($data->total_uang_kopi, 0, ',', '.') : '-' }}
            </td>
            </tr>
        @endif
        <tr>
            ...
            <td class="text-right">
            Rp. {{ $data->total_uang_lembur_minggu > 0 ? number_format($data->total_uang_lembur_minggu, 0, ',', '.') : '-' }}
            </td>
        </tr>
        <tr>
            <td>Uang Makan</td>
            <td class="text-right">
            Rp. {{ $data->total_uang_makan > 0 ? number_format($data->total_uang_makan, 0, ',', '.') : '-' }}
            </td>
        </tr>
        <tr>
            <th class="text-center">TOTAL</th>
            <th class="text-right">
            Rp. {{ $data->total > 0 ? number_format($data->total, 0, ',', '.') : '-' }}
            </th>
        </tr>
    </tbody>
  ...
  <p class="d-inline">Jika ditemukan kesalahan dalam perhitungan harap menghubungi sdri Dwi Ata Yulia atau Rizki Amalia</p>
